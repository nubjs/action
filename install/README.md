...
- uses: actions/setup-node@v4
  with:
    node-version: 22
    cache: npm
- uses: nubjs/action/install@v0     # was: - run: npm ci
- run: npm test
```
...
- uses: actions/setup-node@v4
  with:
    node-version: 22
- uses: nubjs/action/install@v0     # was: pnpm/action-setup, then - run: pnpm install --frozen-lockfile
  with:
    shim: true
...
- `nubjs/action/install@v0`: floating, moves with every `v0.x` release. Breaking changes can land in a `v0.x` release.
- `nubjs/action/install@v0.2.0`: a release, never moves.
